...
for (( i = 0; i < 4; i++ )); do
  if [ $platform = 'linux' ]; then
    sed -i 's#"172800"#"200"#g' $EMONEY_HOME/build/node$i/config/genesis.json
    sed -i 's/timeout_commit = "500ms"/timeout_commit = "1s"/g' $EMONEY_HOME/build/node$i/config/config.toml
    sed -i 's/timeout_propose = "2s"/timeout_propose = "1s"/g' $EMONEY_HOME/build/node$i/config/config.toml
